-- ── Trigger: booking_requests → bookings (v2) ────────────────────────────────
-- Changes vs v1:
--   - When no plate is provided, auto-assigns the vehicle with fewest bookings
--     that day (scales automatically as more vehicles are added to the fleet).
--   - When a plate IS provided, original find-or-create logic is preserved.
...
BEGIN
  ...
  IF v_plate IS NOT NULL THEN
    -- Plate provided: find or create that vehicle
