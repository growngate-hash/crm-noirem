  ...
  useEffect(()=>{
    ...
    async function load(){
      ...
      const [{data:cats,error:ce},{data:svcs,error:se}]=await Promise.all([
        sb.from('service_categories').select('*').eq('is_active',true).eq('user_id',ownerId).order('sort_order'),
        sb.from('services').select('*').eq('is_active',true).eq('user_id',ownerId).order('name'),
      ])
  ...
  if (tenantNotFound) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center',
      ...
      <p style={{ color:'#888580' }}>
        Verifica el link de reservas o contacta a la empresa.
      </p>
    </div>
  )
