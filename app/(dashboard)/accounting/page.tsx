...
export default function AccountingPage() {
  ...
  const th: React.CSSProperties = {
    padding: '10px 14px', textAlign: 'left', color: '#5A5852', fontSize: 11,
    letterSpacing: '1px', fontWeight: 700, borderBottom: '1px solid #F0EFEA',
  }
  ...
  const typeBadgeStyle = (type: string): React.CSSProperties => {
    const map: Record<string, {bg:string; color:string; border:string}> = {
      PAYROLL: { bg:'#FFF3E0', color:'#B45309', border:'#F5B544' },
      PAYMENT: { bg:'#E6F0FA', color:'#0B2A4A', border:'#CBD8E8' },
      INVOICE: { bg:'#E6F4EE', color:'#1A6B40', border:'#A3D4B5' },
      EXPENSE: { bg:'#FBE7E2', color:'#D9533D', border:'#F5B8AE' },
    }
    const t = type?.toUpperCase()
    const s = map[t] ?? { bg:'#F0EFEA', color:'#5A5852', border:'#E5E7EB' }
    return { background: s.bg, border: `1px solid ${s.border}`, color: s.color, borderRadius: 4, padding: '2px 8px', fontSize: 10, fontWeight: 700 }
  }
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, fontFamily: 'Outfit, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ color: '#0B2A4A', fontSize: 24, fontWeight: 800, margin: 0 }}>Contabilidad</h1>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 24, flexWrap: 'wrap' }}>
        {tabs.map(tab => (
          <button
            ...
            style={{
              padding: '8px 16px',
              background: activeTab === tab.id ? '#0B2A4A' : '#FFFFFF',
              color:      activeTab === tab.id ? '#FFFFFF' : '#5A5852',
              border:    `1px solid ${activeTab === tab.id ? '#0B2A4A' : '#F0EFEA'}`,
              borderRadius: 8, fontSize: 12, fontWeight: activeTab === tab.id ? 700 : 600, cursor: 'pointer',
            }}
          >
            {tab.label}
          </button>
              ...
              {journalEntries.length === 0 ? (
                ...
                <>
                  <tr
                    ...
                    style={{ cursor: 'pointer', background: selectedEntry?.id === entry.id ? 'rgba(11,42,74,0.06)' : 'transparent', borderBottom: '1px solid #F0EFEA' }}
                  >
                    <td style={{ ...td, color: '#0B2A4A', fontWeight: 600 }}>{entry.entry_number}</td>
                    <td style={{ ...td, color: '#5A5852' }}>{new Date(entry.entry_date + 'T00:00:00+04:00').toLocaleDateString('en-AE')}</td>
                    <td style={{ ...td, color: '#5A5852' }}>{entry.description}</td>
                    <td style={td}>
                      <span style={typeBadgeStyle(entry.reference_type ?? '')}>
                        {entry.reference_type?.toUpperCase()}
                      </span>
                    </td>
                    <td style={{ ...td, color: '#0B2A4A', fontWeight: 700 }}>AED {parseFloat(entry.total_debit || 0).toFixed(2)}</td>
                    <td style={{ ...td, color: '#0B2A4A', fontWeight: 700 }}>AED {parseFloat(entry.total_credit || 0).toFixed(2)}</td>
                    <td style={td}>
                      <span style={{
                        background: entry.status === 'posted' ? '#E6F4EE' : '#FFF3E0',
                        border:    `1px solid ${entry.status === 'posted' ? '#A3D4B5' : '#F5B544'}`,
                        color:      entry.status === 'posted' ? '#1A6B40' : '#B45309',
                  ...
                  {selectedEntry?.id === entry.id && journalLines.length > 0 && (
                    <tr key={entry.id + '-lines'}>
                      <td colSpan={7} style={{ padding: 0, background: '#F5F4EF' }}>
                        <div style={{ padding: '16px 24px' }}>
                          <div style={{ color: '#0B2A4A', fontSize: 11, fontWeight: 700, letterSpacing: '1px', marginBottom: 10 }}>LÍNEAS DEL ASIENTO</div>
                            ...
                            <tbody>
                              {journalLines.map(line => (
                                <tr key={line.id} style={{ borderTop: '1px solid #1a1a1f' }}>
                                  <td style={{ padding: '8px 12px', color: '#0B2A4A', fontSize: 12 }}>
                                    <span style={{ color: '#0B2A4A', marginRight: 6 }}>{line.account?.code}</span>{line.account?.name}
                                  </td>
                              ...
                              <tr style={{ borderTop: '2px solid #2a2a30' }}>
                                <td colSpan={2} style={{ padding: '8px 12px', color: '#5A5852', fontSize: 11, fontWeight: 700 }}>TOTAL</td>
            ...
            <tbody>
              {trialBalance.length === 0 ? (
                ...
                <tr key={account.code} style={{ borderBottom: '1px solid #1a1a1f' }}>
                  <td style={{ ...td, color: '#0B2A4A', fontWeight: 700 }}>{account.code}</td>
                  <td style={{ ...td, color: '#0B2A4A' }}>{account.name}</td>
                  <td style={td}>
                  ...
                  <td style={{ ...td, color: parseFloat(account.balance) >= 0 ? '#0B2A4A' : '#ef4444', fontWeight: 700 }}>
                    AED {parseFloat(account.balance || 0).toFixed(2)}
                  </td>
                </tr>
              ))}
              <tr style={{ background: '#FFFFFF', borderTop: '2px solid #2a2a30' }}>
                <td colSpan={3} style={{ ...td, color: '#0B2A4A', fontWeight: 800 }}>TOTALES</td>
                ...
                <td style={{ ...td, color: '#0B2A4A', fontWeight: 800 }}>—</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      {!loading && activeTab === 'income_statement' && (
        <div style={{ maxWidth: 700 }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 12, padding: 20, marginBottom: 16 }}>
            ...
            {incomeStatement.filter(a => a.type === 'revenue').length === 0
              ...
              : incomeStatement.filter(a => a.type === 'revenue').map(a => (
                <div key={a.code} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #F0EFEA' }}>
                  <span style={{ color: '#0B2A4A', fontSize: 13 }}>{a.code} — {a.name}</span>
                  <span style={{ color: '#22c55e', fontWeight: 700 }}>AED {parseFloat(a.amount || 0).toFixed(2)}</span>
                </div>
              ))}
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0 0', marginTop: 4 }}>
              <span style={{ color: '#0B2A4A', fontWeight: 800 }}>Total Ingresos</span>
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 12, padding: 20, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0 0', marginTop: 4 }}>
              <span style={{ color: '#0B2A4A', fontWeight: 800 }}>Total Gastos</span>
          ...
          {(() => {
            ...
            return (
              <div style={{ background: utilidad >= 0 ? '#22c55e15' : '#ef444415', border: `2px solid ${utilidad >= 0 ? '#22c55e40' : '#ef444440'}`, borderRadius: 12, padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: '#0B2A4A', fontWeight: 800, fontSize: 16 }}>
      ...
      {!loading && activeTab === 'vat_report' && (
        <div style={{ maxWidth: 700 }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #06b6d420', borderRadius: 12, padding: 20 }}>
            ...
            {vatReport.length === 0 ? (
              ...
              <div key={row.period} style={{ marginBottom: 16, padding: 16, background: '#F5F4EF', borderRadius: 8 }}>
                <div style={{ color: '#0B2A4A', fontWeight: 700, marginBottom: 12 }}>
                ...
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid #1a1a1f' }}>
                    <span style={{ color: '#5A5852', fontSize: 13 }}>{item.label}</span>
      ...
      {!loading && activeTab === 'chart_of_accounts' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div>
              <div style={{ color: '#0B2A4A', fontSize: 16, fontWeight: 800 }}>Plan de Cuentas</div>
              ...
              <tbody>
                {accounts.length === 0 ? (
                  ...
                  <tr key={account.id} style={{ borderBottom: '1px solid #1a1a1f' }}>
                    ...
                    <td style={{ ...td, color: '#5A5852' }}>{account.normal_balance === 'debit' ? '← Débito' : 'Crédito →'}</td>
      ...
      {showNewAccount && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 16, padding: 32, width: '100%', maxWidth: 500, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ color: '#0B2A4A', fontSize: 11, fontWeight: 700, letterSpacing: '2px', marginBottom: 6 }}>PLAN DE CUENTAS</div>
            <div style={{ color: '#0B2A4A', fontSize: 20, fontWeight: 800, marginBottom: 24 }}>Nueva Cuenta Contable</div>

            {/* Código y Nombre */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 12, marginBottom: 16 }}>
              <div>
                <div style={{ color: '#5A5852', fontSize: 11, fontWeight: 700, letterSpacing: '1px', marginBottom: 6 }}>CÓDIGO *</div>
                <input
                  value={newAccount.code}
                  onChange={e => setNewAccount(p => ({ ...p, code: e.target.value }))}
                  placeholder="ej. 5211"
                  style={{ width: '100%', padding: '10px 14px', background: '#F5F4EF', border: '1px solid #F0EFEA', borderRadius: 8, color: '#0B2A4A', fontSize: 13, outline: 'none', boxSizing: 'border-box' }}
                />
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                {[
                  ...
                  <button
                    ...
                    style={{
                      padding: 10,
                      background: newAccount.type === type.value ? type.color + '25' : '#F5F4EF',
                      border: `2px solid ${newAccount.type === type.value ? type.color : '#F0EFEA'}`,
                      borderRadius: 8,
              ...
              <div style={{ color: '#555', fontSize: 11, marginTop: 8, textAlign: 'center' }}>
                Saldo normal: <span style={{ color: '#0B2A4A', fontWeight: 700 }}>
            ...
            {/* Cuenta padre */}
            <div style={{ marginBottom: 16 }}>
              <div style={{ color: '#5A5852', fontSize: 11, fontWeight: 700, letterSpacing: '1px', marginBottom: 6 }}>
              ...
              <select
                ...
                style={{ width: '100%', padding: '10px 14px', background: '#F5F4EF', border: '1px solid #F0EFEA', borderRadius: 8, color: newAccount.parent_id ? '#0B2A4A' : '#666', fontSize: 13, outline: 'none' }}
              >
            ...
            {newAccount.code && newAccount.name && (
              <div style={{ background: '#F5F4EF', border: '1px solid rgba(201,168,76,0.3)', borderRadius: 8, padding: '12px 16px', marginBottom: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: '#0B2A4A', fontWeight: 700, fontFamily: 'monospace' }}>{newAccount.code}</span>
                <span style={{ color: '#0B2A4A', fontSize: 13 }}>{newAccount.name}</span>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{ flex: 1, padding: 13, background: 'transparent', border: '1px solid #F0EFEA', borderRadius: 10, color: '#5A5852', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}
              >
                CANCELAR
              </button>
              <button
                ...
                style={{ flex: 2, padding: 13, background: '#0B2A4A', color: '#F5F4EF', border: 'none', borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: 'pointer', opacity: savingAccount || !newAccount.code || !newAccount.name ? 0.6 : 1 }}
              >
      ...
      {showManualEntry && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div style={{ background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 16, padding: 32, width: '100%', maxWidth: 700, maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ color: '#0B2A4A', fontSize: 11, fontWeight: 700, letterSpacing: '2px', marginBottom: 8 }}>ASIENTO CONTABLE MANUAL</div>
            <div style={{ color: '#0B2A4A', fontSize: 20, fontWeight: 800, marginBottom: 24 }}>Nuevo Asiento</div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 20 }}>
              <div>
                <div style={{ color: '#5A5852', fontSize: 11, fontWeight: 700, letterSpacing: '1px', marginBottom: 6 }}>DESCRIPCIÓN *</div>
                <input
              ...
              <div>
                <div style={{ color: '#5A5852', fontSize: 11, fontWeight: 700, letterSpacing: '1px', marginBottom: 6 }}>FECHA *</div>
                <input
                  type="date"
            ...
            <div style={{ color: '#5A5852', fontSize: 11, fontWeight: 700, letterSpacing: '1px', marginBottom: 10 }}>LÍNEAS DEL ASIENTO</div>
            <div style={{ background: '#F5F4EF', borderRadius: 8, overflow: 'hidden', marginBottom: 12 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: '#FFFFFF' }}>
                    ...
                    <tr key={i} style={{ borderTop: '1px solid #1a1a1f' }}>
                      <td style={{ padding: '6px 8px' }}>
                        <select
                          ...
                          style={{ width: '100%', padding: '6px 8px', background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 6, color: '#0B2A4A', fontSize: 12, outline: 'none' }}
                        >
                      ...
                      <td style={{ padding: '6px 8px' }}>
                        <input
                          ...
                          style={{ width: '100%', padding: '6px 8px', background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 6, color: '#0B2A4A', fontSize: 12, outline: 'none', boxSizing: 'border-box' }}
                        />
                      </td>
                      <td style={{ padding: '6px 8px' }}>
                        <input
                          ...
                          placeholder="0.00"
                          style={{ width: 80, padding: '6px 8px', background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 6, color: '#22c55e', fontSize: 12, outline: 'none' }}
                        />
                      </td>
                      <td style={{ padding: '6px 8px' }}>
                        <input
                          ...
                          placeholder="0.00"
                          style={{ width: 80, padding: '6px 8px', background: '#FFFFFF', border: '1px solid #F0EFEA', borderRadius: 6, color: '#ef4444', fontSize: 12, outline: 'none' }}
                        />
                      </td>
                      <td style={{ padding: '6px 8px' }}>
            ...
            <button
              ...
              style={{ background: 'transparent', border: '1px dashed #2a2a30', borderRadius: 8, color: '#5A5852', padding: '8px 16px', fontSize: 12, cursor: 'pointer', marginBottom: 20, width: '100%' }}
            >
              + Agregar línea
            </button>
            ...
            {(() => {
              ...
              return (
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 16px', marginBottom: 20, background: balanced ? '#22c55e15' : '#ef444415', border: `1px solid ${balanced ? '#22c55e40' : '#ef444440'}`, borderRadius: 8 }}>
                  <span style={{ color: '#5A5852', fontSize: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button
                ...
                style={{ flex: 2, padding: 13, background: '#0B2A4A', border: 'none', borderRadius: 10, color: '#F5F4EF', fontSize: 13, fontWeight: 800, cursor: 'pointer' }}
              >
